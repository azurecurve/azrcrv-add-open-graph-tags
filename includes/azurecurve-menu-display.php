...
if ( ! function_exists( 'azrcrv_display_azurecurve_menu' ) ) {
	function azrcrv_display_azurecurve_menu() {
		...
		echo '<div id="azrcrv-general" class="wrap">';
			...
			echo '<h1>';
				echo '<a href="' . esc_url_raw( azurecurve\AddOpenGraphTags\DEVELOPER_URL_RAW ) . '"><img src="' . esc_url_raw( plugins_url( '../assets/images/logo.svg', __FILE__ ) ) . '" style="padding-right: 6px; height: 20px; width: 20px;" alt="azurecurve" /></a>';
				echo esc_html( azurecurve\AddOpenGraphTags\DEVELOPER_SHORTNAME ) . ' ClassicPress Plugins';
			echo '</h1>';

			$plugin_array = get_option( 'azrcrv-plugin-menu' );

			$tab_active_label = esc_html__( 'Active Plugins', 'azrcrv-aogt' );
			$active_plugins   = '';
		...
		foreach ( $plugin_array as $plugin_name => $plugin_details ) {
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							<p>' .
								sprintf( esc_html__( '%1$s was one of the first plugin developers to start developing for ClassicPress; all plugins are available from %2$s and are integrated with the %3$s plugin for fully integrated, no hassle, updates.', 'azrcrv-aogt' ), '<strong>' . azurecurve\AddOpenGraphTags\DEVELOPER_NAME . '</strong>', azurecurve\AddOpenGraphTags\DEVELOPER_URL, '<a href="https://directory.classicpress.net/plugins/update-manager/">Update Manager</a>' )
							. '</p>
							<p>' .
								sprintf( esc_html__( 'The %s plugins active on your site are:', 'azrcrv-aogt' ), '<strong>' . azurecurve\AddOpenGraphTags\DEVELOPER_NAME . '</strong>' )
							. '</p>
			...
			$tab_other_label = esc_html__( 'Other Available Plugins', 'azrcrv-aogt' );
			$other_plugins   = '';
		...
		if ( $countofplugins == 0 ) {
			$other_plugins .= sprintf( esc_html__( 'Congratulations! You\'re using all of the %s plugins.', 'azrcrv-aogt' ), '<strong>' . azurecurve\AddOpenGraphTags\DEVELOPER_NAME . '</strong>' );
		}
				...
				<table class="form-table azrcrv-settings">
					...
					<tr>
						...
						<td scope="row" colspan=2>
							...
							<p>' .
								sprintf( esc_html__( 'The other plugins available from %s are:', 'azrcrv-aogt' ), '<strong>' . azurecurve\AddOpenGraphTags\DEVELOPER_NAME . '</strong>' )
							. '</p>
